...
def move():
    q = deque([(srr,src,sbr,sbc,0,1),(srr,src,sbr,sbc,0,2),(srr,src,sbr,sbc,0,0),(srr,src,sbr,sbc,0,3)])
    visited.add((srr,src,sbr,sbc))
    while q:
        rr,rc,br,bc,depth,look = q.popleft()
        if depth == 10:
            return -1
        for i in range(4):
            if i != look:
                ...
                if nbr == -1 and nbc == -1:
                    continue
                if 0<nrr<n-1 and 0<nrc<m-1 and 0<nbr<n-1 and 0<nbc<m-1 and (nrr,nrc,nbr,nbc) not in visited:
                    #todo r과 b를 따로 계산하여 r이나 b가 못가면 갈수 있는것만 움직이게 하는 기능 구현하기
                    
                    visited.add((nrr,nrc,nbr,nbc))
                    q.append((nrr,nrc,nbr,nbc,depth+1,i))
